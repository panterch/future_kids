...
describe SelfRegistrationsController do
  ...
  context 'enabled' do
    ...
    context 'create' do
      ...
      context 'teacher' do
        let(:params) do
          { teacher: attributes_for(:teacher).merge(type: 'Teacher', school_id: create(:school).id),
            terms_of_use: { accepted: 'yes' } }
        end
        ...
        it "can't be created if not accepted terms of conditions" do
          post :create,
               params: params.deep_merge({ teacher: { email: attributes_for(:teacher)[:email] } }).except(:terms_of_use)
      ...
      context 'mentor' do
        let(:params) do
          { mentor: attributes_for(:mentor).merge(type: 'Mentor', school_id: create(:school).id),
            terms_of_use: { accepted: 'yes' } }
        end
        ...
        it "can't force state" do
          post :create,
               params: params.deep_merge({ mentor: { state: :accepted, email: attributes_for(:mentor)[:email] } })
          expect(response).to redirect_to action: :success, type: 'mentor'
        ...
        it "can't be created if not accepted terms of conditions" do
          post :create,
               params: params.deep_merge({ mentor: { email: attributes_for(:mentor)[:email] } }).except(:terms_of_use)
          expect(response).to have_http_status :success
        end
      end
